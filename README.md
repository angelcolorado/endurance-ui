...
- `fakeAsync`/`tick` from `@angular/core/testing` requires Zone.js patching of the event loop. The project uses Vitest, which is zoneless — `fakeAsync` silently fails to drain the microtask queue, leaving observables un-emitted and causing `afterEach(() => httpMock.verify())` to cascade failures across all tests in the suite. All timer-dependent tests use `vi.useFakeTimers()` + `vi.advanceTimersByTime()` instead; all time-delayed mock observables use a `Subject` that emits synchronously under test control.
- `ng-reflect-*` attributes (e.g. `ng-reflect-router-link`) are Ivy debug attributes emitted only in development mode. They are absent in Vitest builds. Router link assertions must use the rendered `href` attribute on the native element instead.
